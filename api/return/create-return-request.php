        ...
        if (is_array($files['name'])) {
            for ($i = 0; $i < count($files['name']); $i++) {
                if ($files['error'][$i] === UPLOAD_ERR_OK) {
                    ...
                    // Upload to Cloudinary using wrapper function
                    $result = uploadToCloudinary($tmpName, 'darling/returns');
                   
                    if (!empty($result['success'])) {
                        $proofImagesUrls[] = $result['url'];
                    } else {
            ...
            if ($files['error'] === UPLOAD_ERR_OK) {
                $result = uploadToCloudinary($files['tmp_name'], 'darling/returns');
