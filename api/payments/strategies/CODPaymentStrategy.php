...
class CODPaymentStrategy implements PaymentStrategy
{
    ...
    public function processPayment(array $orderData): array
    {
        try {
            ...
            $paymentId = $this->createPaymentRecord($orderId, $finalAmount);
            $this->pdo->commit();
            return [
                'success' => true,
                'data' => [
    ...
    private function createPaymentRecord(int $orderId, float $amount): int
    {
        ...
        $stmtPayment->execute([$orderId, $amount]);
